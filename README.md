...
- **Choose the folder** each new attachment goes into, per note or per anything else a pattern can express — one folder beside every note, a folder per note, a folder per year. Or turn on `shouldFollowObsidianAttachmentLocation` and let Obsidian's own *Default location for new attachments* decide, keeping everything else this plugin does. While the plugin's pattern is in charge, Obsidian's own settings page says so instead of showing a value that is not in effect. [01 Attachment folder location](<./demo-vault/01 Attachment folder location.md>)
